...
export function PharmacistDashboard({ onNavigate: propNavigate }) {
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
                        Daily dispensary stock operations, batch monitoring, and expiry surveillance
                    </p>
                </div>
                <div className="flex items-center gap-2.5">
                    <Button variant="primary" onClick={() => navigate('/dispense')} className="shadow-sm text-xs">
                        <Package className="h-4 w-4" /> Open POS Dispense
                    </Button>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <KpiCard
                    label="In Stock Active Catalog"
                    value={medicines.length}
                    icon={<Boxes size={22} />}
                    badgeColor="primary"
                    onView={() => navigate('/inventory')}
                    actionText="View Inventory Batches →"
                />
                <KpiCard
                    ...
                    actionText="Inspect Low Stock Catalog →"
                />
                <KpiCard
                    ...
                    actionText="Open Expiry Tracker →"
                />
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <Card className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm">
                    ...
                    <div className="px-3 pb-3 space-y-1.5">
                        {expiringSoon.length === 0 ? (
                            <p className="text-xs text-slate-400 text-center py-8">No batches expiring within the next 30 days.</p>
                        ) : (
                ...
                <Card className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm">
                    <CardHeader
                        ...
                        subtitle="Medicines below minimum stock threshold"
                        action={
                    ...
                    <div className="px-3 pb-3 space-y-1.5">
                        {lowStock.length === 0 ? (
                            <p className="text-xs text-slate-400 text-center py-8">All stock levels are optimal.</p>
                        ) : (
                            lowStock.slice(0, 5).map((m) => {
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <div className="min-w-0 flex-1">
                                            <p className="text-sm font-semibold text-slate-900 dark:text-white truncate">{medName}</p>
                                            <p className="text-xs text-slate-500 dark:text-slate-400">Reorder recommended</p>
                                        </div>
